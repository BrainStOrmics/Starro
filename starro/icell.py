...
def _mask_cells_from_stain(
    X: np.ndarray, otsu_classes: int = 3, otsu_index: int = 0, mk: int = 7
) -> np.ndarray:
...
def _mask_nuclei_from_stain(
    ...
    if X.dtype != np.uint8:
        ...
        local_mask = X > filters.threshold_local(
            X, block_size=local_k, method="gaussian", offset=offset
        )
    else:
...
def _score_pixels(
    ...
    res = utils.conv2d(
        X, k, mode="gauss" if method in ("gauss", "moran") else "circle", bins=bins
    )
    ...
    if method == "gauss":
        ...
        res = moran.run_moran(
            res, mask=None if bins is None else bins > 0, **moran_kwargs
        )
        # Rescale
        res /= res.max()
    else:
        ...
        if "em" in method:
            ...
            conditional_func = partial(
                em.conditionals, em_results=em_results, bins=bins
            )
        else:
            ...
            conditional_func = partial(
                vi.conditionals, vi_results=vi_results, bins=bins
            )
...
def score_and_mask_pixels(
    ...
    scores = _score_pixels(
        X, k, method, moran_kwargs, em_kwargs, vi_kwargs, bp_kwargs, certain_mask, bins
    )
