    ...
    while indices_to_add:
        ...
        if shift > 1:
            ...
            for ov_label in overlapping:
                ...
                # Shift
                xs[ov_idx] = min(
                    max(0, round(ov_x + ((diff_x + 1e-5) / distance * shift))), shape[0]
                )
                ys[ov_idx] = min(
                    max(0, round(ov_y + (diff_y + 1e-5) / distance * shift)), shape[1]
                )
        i += 1
...
def simulate_cells(
    ...
    axes1 = stats.loguniform.rvs(
        axis1_range[0], axis1_range[1], size=n, random_state=rng
    ).astype(np.int32)
    axes2 = stats.loguniform.rvs(
        axis2_range[0], axis2_range[1], size=n, random_state=rng
    ).astype(np.int32)
