...
def shift_cells(cells, labels, max_iter, seed, shift_length=10):
    ...
    c = 0
    np.random.seed(seed)
    center_shifts = np.random.randint(
        -shift_length, shift_length + 1, 2 * max_iter + 2
    ).reshape(-1, 2)
...
def select_area(area_df, cell_num, seed):
    ...
    areas = np.array(
        [
            ...
            for i in range(int(row["cell_num"]))
        ]
    )
...
def add_sig_to_cell(labels, cell_mean_df, bg_mean_df, seed):
    ...
    cells = np.array(
        [
            index
            for index, row in cell_mean_df.iterrows()
            for i in range(int(row["prob"] * 1000))
        ]
    )
    ...
    bgs = np.array(
        [
            index
            for index, row in bg_mean_df.iterrows()
            for i in range(int(row["prob"] * 1000))
        ]
    )
