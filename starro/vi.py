...
class NegativeBinomialMixture(PyroModule):
    ...
    def dist(self, assignment, train=False):
        ...
        return dist.ZeroInflatedNegativeBinomial(
            counts[assignment],
            logits=logits[assignment],
            gate_logits=z[assignment],
            validate_args=False,
        )
    ...
    def init_mean_variance(self, w, mu, var):
        ...
        if self.zero_inflated:
            self.z = PyroParam(
                probs_to_logits(torch.zeros(self.n).float(), is_binary=True)
            )
    ...
    def forward(self, x):
        with pyro.plate("x", size=len(x)):
            assignment = pyro.sample(
                "assignment", self.assignment(True), infer={"enumerate": "parallel"}
            )
    ...
    def train(self, n_epochs: int = 500):
        ...
        guide = AutoDelta(
            poutine.block(self, expose=list(self.get_params(True).keys()))
        )
    ...
    def conditionals(params, x, use_weights=False):
        ...
        dists = [
            dist.ZeroInflatedNegativeBinomial(
                c, logits=l, gate_logits=torch.tensor(_z), validate_args=False
            )
        ...
        means = [
            (1 - logits_to_probs(dist.gate_logits, is_binary=True))
            * dist.base_dist.mean
            for dist in dists
        ]
    ...
    if "counts" not in vi_results:
        if bins is None:
            raise SegmentationError(
                "`vi_results` indicate binning was used, but `bins` was not provided"
            )
...
def run_vi(
    ...
    bins: Optional[np.ndarray] = None,
    params: Union[
        Dict[str, Tuple[float, float]], Dict[int, Dict[str, Tuple[float, float]]]
    ] = dict(w=(0.5, 0.5), mu=(10.0, 300.0), var=(20.0, 400.0)),
    zero_inflated: bool = False,
    seed: Optional[int] = None,
) -> Union[
        ...
        for label in np.unique(bins):
            if label > 0:
                ...
                if set(_params.keys()) != {"w", "mu", "var"}:
                    raise SegmentationError(
                        "`params` must contain exactly the keys `w`, `mu`, `var`."
                    )
    else:
        samples[0] = X.flatten()
    ...
    for label, _samples in samples.items():
        _downsample = (
            int(len(_samples) * downsample)
            if downsample_scale
            else int(downsample * (len(_samples) / total))
        )
