...
class StarroAdataKeyManager:
    ...
    def select_layer_data(
        ...
        if layer is None:
            layer = StarroAdataKeyManager.X_LAYER
        res_data = None
        if layer == StarroAdataKeyManager.X_LAYER:
            res_data = adata.X
        else:
    ...
    def get_adata_type(adata: AnnData) -> str:
        return adata.uns[StarroAdataKeyManager.ADATA_TYPE_KEY]

    def adata_is_type(adata: AnnData, t: str) -> bool:
        return StarroAdataKeyManager.get_adata_type(adata) == t
    ...
    def check_adata_is_type(t: str, argname: str = "adata", optional: bool = False):
        def decorator(func):
            ...
            def wrapper(*args, **kwargs):
                ...
                passing = (
                    all(
                        StarroAdataKeyManager.adata_is_type(_adata, t)
                        for _adata in adata
                    )
                    if isinstance(adata, (list, tuple))
                    else StarroAdataKeyManager.adata_is_type(adata, t)
                ...
                if (not optional or adata is not None) and not passing:
                    if isinstance(adata, (list, tuple)):
                        ...
                        raise ConfigurationError(
                            ...
                            f"`{StarroAdataKeyManager.get_adata_type(adata)}` type."
                        )
                    else:
                        raise ConfigurationError(
                            f"AnnData is not AnnData object, but {type(adata)}."
                        )
    ...
    def init_adata_type(adata: AnnData, t: Optional[str] = None):
        lm.main_info_insert_adata_uns(StarroAdataKeyManager.ADATA_TYPE_KEY)
        if t is None:
            t = StarroAdataKeyManager.ADATA_DEFAULT_TYPE
        adata.uns[StarroAdataKeyManager.ADATA_TYPE_KEY] = t

    def init_uns_pp_namespace(adata: AnnData):
        lm.main_info_insert_adata_uns(StarroAdataKeyManager.UNS_PP_KEY)
        if StarroAdataKeyManager.UNS_PP_KEY not in adata.uns:
            adata.uns[StarroAdataKeyManager.UNS_PP_KEY] = {}

    def init_uns_spatial_namespace(adata: AnnData):
        lm.main_info_insert_adata_uns(StarroAdataKeyManager.UNS_SPATIAL_KEY)
        if StarroAdataKeyManager.UNS_SPATIAL_KEY not in adata.uns:
            adata.uns[StarroAdataKeyManager.UNS_SPATIAL_KEY] = {}

    def set_uns_spatial_attribute(adata: AnnData, key: str, value: object):
        if StarroAdataKeyManager.UNS_SPATIAL_KEY not in adata.uns:
            StarroAdataKeyManager.init_uns_spatial_namespace(adata)
        adata.uns[StarroAdataKeyManager.UNS_SPATIAL_KEY][key] = value

    def get_uns_spatial_attribute(adata: AnnData, key: str) -> object:
        return adata.uns[StarroAdataKeyManager.UNS_SPATIAL_KEY][key]

    def has_uns_spatial_attribute(adata: AnnData, key: str) -> bool:
        return key in adata.uns[StarroAdataKeyManager.UNS_SPATIAL_KEY]

    def get_agg_bounds(adata: AnnData) -> Tuple[int, int, int, int]:
        """Get (xmin, xmax, ymin, ymax) for AGG type anndatas."""
        atype = StarroAdataKeyManager.get_adata_type(adata)
        if atype != StarroAdataKeyManager.ADATA_AGG_TYPE:
            raise ConfigurationError(f"AnnData has incorrect type: {atype}")
        return (
            int(adata.obs_names[0]),
            int(adata.obs_names[-1]),
            int(adata.var_names[0]),
            int(adata.var_names[-1]),
        )


SKM = StarroAdataKeyManager
...
def starro_theme(background="white"):
...
def config_starro_rcParams(
    ...
    """Configure matplotlib.rcParams to starro defaults (based on ggplot style and scanpy).
...
def set_figure_params(
    ...
    Args:
        starro: `bool` (default: `True`)
            Init default values for :obj:`matplotlib.rcParams` suited for starro.
...
def set_pub_style(scaler: float = 1) -> None:
    ...
    set_figure_params("starro", background="white")
