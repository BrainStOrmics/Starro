...
def _stardist(
    img: np.ndarray,
    model: Union[
        Literal["2D_versatile_fluo", "2D_versatile_he", "2D_paper_dsb2018"],
        "StarDist2D",
    ] = "2D_versatile_fluo",
    **kwargs,
) -> np.ndarray:
    ...
    **kwargs,
    ...
    if StarDist2D is None:
        raise ModuleNotFoundError(
            "Please install StarDist by running `pip install stardist`."
        )
    if tilesize <= 0 and min_overlap:
        raise SegmentationError(
            "Positive `tilesize` must be provided when `min_overlap` is used."
        )
    ...
    if not min_overlap:
        n_tiles = (
            (math.ceil(img.shape[0] / tilesize), math.ceil(img.shape[1] / tilesize))
            if tilesize > 0
            else (1, 1)
        )
