...
def replace_labels(
    adata: AnnData, layer: str, mapping: Dict[int, int], out_layer: Optional[str] = None
):
    """Replace labels according to mapping.
...
def find_peaks_with_erosion(
    ...
    if (
        _layer1 not in adata.layers
        and _layer2 not in adata.layers
        and layer not in adata.layers
    ):
            ...
            for i, (_expanded, _new_areas) in zip(
                ...
                parallel(
                    delayed(_expand)(
                        ...
                        sm,
                        int(i - 1 >= 0),
                        sl.shape[0] - int(i + step + 1 < labels.shape[0]),
                    )
...
def _label_connected_components(
    ...
    for label, area in enumerate(areas):
        if label > 0:
            ...
            if (
                seed_labels is not None
                and (
                    seed_labels[top : top + height, left : left + width][label_mask] > 0
                ).any()
            ):
                continue
...
def label_connected_components(
    ...
    labels = _label_connected_components(
        mask, area_threshold, k, min_area, n_iter, distance, max_area, seed_labels
    )
    out_layer = out_layer or SKM.gen_new_layer_key(layer, SKM.LABELS_SUFFIX)
    SKM.set_layer_data(adata, out_layer, labels)
...
def find_peaks_from_mask(
    ...
    if mask.dtype != np.dtype(bool):
        raise SegmentationError(
            f"Only boolean masks are supported for this function, but got {mask.dtype} instead."
        )
