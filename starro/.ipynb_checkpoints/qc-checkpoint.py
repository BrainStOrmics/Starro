...
def select_qc_regions(
    ...
    use_scale: bool = True,
    absolute: bool = False,
    weight_func: Optional[Callable[[AnnData], float]] = lambda adata: np.log1p(
        adata.X.sum()
    ),
):
    ...
    if not regions:
        ...
        indices = np.dstack(
            np.meshgrid(
                np.arange(0, adata.n_obs - size, size),
                np.arange(0, adata.n_vars - size, size),
            )
        ).reshape(-1, 2)

        if indices.shape[0] == 0:
            raise SegmentationError(
                "No possible regions found. This may indicate the `size` argument is to big."
            )
        ...
        if weight_func is None:
            ...
            idx = rng.choice(
                np.arange(indices.shape[0]), n, replace=False, p=p / p.sum()
            )
        choices = indices[idx]
        ...
        scale = (
            SKM.get_uns_spatial_attribute(adata, SKM.UNS_SPATIAL_SCALE_KEY) * binsize
        )
        ...
        for i, region in enumerate(regions):
            if len(region) == 4:
                ...
                raise SegmentationError(
                    "`regions` must be a list of 4-element or 2-element tuples."
                )
            ...
            if (
                xmin < adata_bounds[0]
                or xmax >= adata_bounds[1]
                or ymin < adata_bounds[2]
                or ymax >= adata_bounds[3]
            ):
                lm.main_warning(
                    f"Region {region} is out of bounds. It will be clipped into bounds."
                )
...
def _generate_random_labels(
    shape: Tuple[int, int], areas: List[int], seed: Optional[int] = None
) -> np.ndarray:
    n = np.prod(shape)
    if sum(areas) > n:
