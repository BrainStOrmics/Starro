...
def binary_morani_result(
    ...
    if pvalue_cutoff == None:
        if method == "otsu":
            ...
            if isinstance(tissue_mask, np.ndarray):
                ...
                p2 = p.flatten()
            pvalue_cutoff = threshold_otsu(
                hist=(np.bincount(p2), np.arange(len(np.bincount(p2))))
            )
    ...
    if isinstance(tissue_mask, np.ndarray):
        cell_mask = np.where(
            (p_cell_mask == 255) & (c >= c_cutoff) & (tissue_mask > 0), 255, 0
        ).astype(np.uint8)
    else:
        cell_mask = np.where((p_cell_mask == 255) & (c >= c_cutoff), 255, 0).astype(
            np.uint8
        )
