    ...
    if isinstance(em_results, dict):
        if bins is None:
            raise SegmentationError(
                "`em_results` indicate binning was used, but `bins` was not provided"
            )
...
def run_em(
    ...
    for label, _samples in samples.items():
        ...
        if len(_samples) > _downsample:
            ...
            _samples = rng.choice(
                _samples, _downsample, replace=False, p=weights / weights.sum()
            )
    ...
    for label, (res_w, res_r, res_p) in zip(
        ...
        ParallelWithProgress(
            n_jobs=config.n_threads, total=len(final_samples), desc="Running EM"
        )(
            delayed(nbn_em)(
                final_samples[label],
                max_iter=max_iter,
